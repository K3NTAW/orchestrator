...
`cache_downgrade_tokens` (default 600), and its routing reason is the generic fallback `ambiguous`.
...
cache-adjusted level differs from their presented level. With no matching rows it prints
