...
1. Read `.orchestrator/plan.md` + `memory/*.md`; `bus_read(status_not="done")`. If plan.md has a goal → skill `resume`.
...
6. Execute: `status()`; if Codex is available, `codex(prompt, cwd="wt/<id>")` per task; `bus_claim(id, "codex")`; store threadId. Fix loop ≤5 via `codex-reply` using `.orchestrator/prompts/fix-delta.md`. Codex cooling → refill the pipeline, never wait idle.
7. Accept only when `.claude/hooks/tests-green.sh wt/<id>` exits 0 in your own shell. Review per complexity policy. `merge(id)`.
8. Retrospective: overwrite memory files with a dated entry, then complete the GOAL task (hook-enforced). Open PR goal/<parent> → main.
