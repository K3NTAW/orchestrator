...
def scores(card, min_runs=5):
    """success = merged/(merged+failed) over resolved tasks; neutral (1.0) below min_runs samples so a new or
    lightly-used executor isn't penalized on noise. A hot quota group (>=3 usage-limit hits today) is halved."""
    out = {}
    for eid, r in card.items():
        total = r.get("merged", 0) + r.get("failed", 0)
        score = 0.5 + r["merged"] / total if total >= min_runs and total > 0 else 1.0
