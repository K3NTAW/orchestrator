...
REQUIREMENTS = [
    ...
    {"id": "adaptive-parallelism-p5", "program": "adaptive-parallelism", "priority": "P5", "title": "Duration-aware scheduling", "experimental": False, "files": ["orchestrator/duration.py"], "tests": ["tests/test_duration.py::test_sufficient_history_uses_class_executor_median"], "config": ["scheduler.duration_mode"], "telemetry": ["scheduler.duration_estimate"]},
    {"id": "adaptive-parallelism-p6", "program": "adaptive-parallelism", "priority": "P6", "title": "Capacity-aware scheduling", "experimental": False, "files": ["orchestrator/capacity.py"], "tests": ["tests/test_capacity.py::test_imminent_critical_task_reserves_last_slot"], "config": ["scheduler.concurrency_mode"], "telemetry": ["scheduler.capacity"]},
    ...
    {"id": "adaptive-parallelism-p11", "program": "adaptive-parallelism", "priority": "P11", "title": "Merge-pressure awareness", "experimental": False, "files": ["orchestrator/daemon.py"], "tests": ["tests/test_daemon.py::test_wave_row_carries_priority_order"], "config": ["scheduler.merge_pressure_mode"], "telemetry": ["scheduler.merge_pressure"]},
    ...
    {"id": "adaptive-parallelism-p16", "program": "adaptive-parallelism", "priority": "P16", "title": "Critical-path model allocation", "experimental": False, "files": ["orchestrator/critical_path.py"], "tests": ["tests/test_critical_path.py::test_rank_prefers_longer_downstream_chain"], "config": ["scheduler.allocation.mode"], "telemetry": ["scheduler.allocation"], "mode_key": "scheduler.allocation.mode"},
    {"id": "adaptive-parallelism-p17", "program": "adaptive-parallelism", "priority": "P17", "title": "Selective speculative execution", "experimental": True, "files": ["orchestrator/speculation.py"], "tests": ["tests/test_speculation.py::test_default_mode_off_never_speculates"], "config": ["scheduler.speculation.mode"], "telemetry": ["scheduler.speculation"], "mode_key": "scheduler.speculation.mode"},
    {"id": "adaptive-parallelism-p18", "program": "adaptive-parallelism", "priority": "P18", "title": "Workflow strategy learning", "experimental": False, "files": ["orchestrator/strategy.py"], "tests": ["tests/test_strategy.py::test_shadow_recommendation_prefers_evidence_without_changing_policy"], "config": ["scheduler.strategy.mode"], "telemetry": ["scheduler.strategy"], "mode_key": "scheduler.strategy.mode"},
...
def status(req, root, cfg=None):
    ...
    if mode == "shadow":
        return "implemented_shadow"
    if mode in ("off", False):
        return "implemented_off"
    return "implemented_active"
